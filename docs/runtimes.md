...
MQTT functions connect to a broker and subscribe to `{prefix}/{target}` (e.g., `fnkit/my-handler`). See the [MQTT deep dive](mqtt.md) for configuration, environment variables, and broker setup.
...
← [Back to README](../README.md) · [MQTT deep dive →](mqtt.md)
